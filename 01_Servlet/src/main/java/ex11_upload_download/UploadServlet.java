...
public class UploadServlet extends HttpServlet {
	...
	*/
	...
	// 업로드 경로(서버의 RealPath를 사용하거나 일반 경로를 사용할 수 있다.)
		...
		// 업로드 진행하기 (HttpServletRequest를 이용해서 MultipartRequest 객체를 생성하면 업로드가 곧바로 진행된다.)
		...
		//업로드 결과(요청 파라미터들 : MultipartRequest multipartRequest를 이용해서 파라미터를 가져와야 한다.)
		String uploader = multipartRequest.getParameter("uploader");
		String originName = multipartRequest.getOriginalFileName("filename"); 	//원래 파일 첨부명
		String filesystemName = multipartRequest.getFilesystemName("filename");	// 저장된 첨부 파일명(같을수도,다를수도있다.)
